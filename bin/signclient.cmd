...
for /f "tokens=*" %%a in ('dir /b /s %SIGNSRV_HOME%\lib\EJBCA-cesecore-common-*.jar') do set JAR_CESECORE=%%a
...
if not exist %JAR_CESECORE%  (
    echo You must build cesecore module of SignServer before using the CLI.
    goto end
)
...
set CLASSPATH=%SIGNSRV_HOME%\conf;%SIGNSRV_HOME%\bin;%JAR%;%JAR_CESECORE%;%J2EE_CP%;%SIGNSRV_HOME%\res\deploytools\cesecore;%OPTIONAL_CLASSPATH%
rem echo %CLASSPATH%
